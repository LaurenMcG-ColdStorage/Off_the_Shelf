...
function* addPlayHistory(action) {
    try{
        ...
        const postResponse = yield axios.post('/api/history', userData); //Send user data for storage in history table
...
function* getPlayHistory(action) {
    try{
        ...
        const getReponse = yield axios.get( `/api/history/${userData}`); //Get user history data from table
